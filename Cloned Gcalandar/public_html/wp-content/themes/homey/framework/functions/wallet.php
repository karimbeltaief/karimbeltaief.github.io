...
if(!function_exists('homey_add_earning')) {
    function homey_add_earning($reservation_id) {
        ...
        //echo "<pre>"; print_r($reservation_meta); die();
        ...
        //echo "<pre>"; print_r($prices_array); die();
        $total_price = $prices_array['total_price'];
        //$services_fee = $prices_array['services_fee'];
        //$security_deposit = $prices_array['security_deposit'];

        //Change By Parth
        $services_fee = isset($prices_array['services_fee']) ? $prices_array['services_fee'] : '';
        $security_deposit = isset($prices_array['security_deposit']) ? $prices_array['security_deposit'] : '';
        //End Change By Parth
    ...
    function homey_add_payout() {
        ...
        if(empty($homey_is_host_payout)) {
            echo json_encode( 
                array( 
                    'success' => false,
                    'msg' => esc_html__('Please setup your payout method', 'homey')
                )
            );
            wp_die();
        }
    ...
    function homey_make_host_adjustments() {
        ...
        if($order_id) {
            ...
            if($adj_action == 'deduct_money') {
                homey_adjust_host_available_balance($host_id, $adj_amount);
                ...
                homey_addto_host_available_balance($host_id, $adj_amount);
    ...
    function homey_save_payout_method_info() {
        ...
        if(!empty($required)) {
            ...
            echo json_encode( 
                array( 
                    'success' => true, 
                    'message' => esc_html__('Information saved successfully.'.$userID, 'homey'),
                    'req' => ''
                ) 
             );
             wp_die();
