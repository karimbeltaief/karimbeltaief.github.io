...
$ac_wallet = $ac_dash = $ac_profile = $ac_fav = $ac_listings = $ac_invoices = $ac_msgs = $ac_submission = $ac_reserv = '';
    ...
    <div class="navi">
        <ul class="board-panel-menu">
            ...
            if( !empty($dashboard_reservations) ) {
                $class = "msg-alert";
                $display = "none";
                $badge = '<span class="' . $class . '" style="display: none;"></span>';
                echo '<li class="'.esc_attr($ac_reserv).' reservation-menu">
                    <a href="'.esc_url($dashboard_reservations).'">'.$homey_local['m_reservation_label'].$badge.'</a>
                </li>';
            }
                ...
                if(homey_is_host()) {
                    if( !empty($dashboard_wallet) ) {
                        echo '<li class="'.esc_attr($ac_wallet).' has-child">
                            ...
                            <ul>
                                <li><a href="'.esc_url($payouts_setup_page).'">'.esc_html__('Setup', 'homey').'</a></li>
