...
SkedTape.prototype = {
	...
	renderTimeWrap: function(oldScroll, opts) {
		var $hours = this.renderHours(opts);
	...
	renderHours: function(opts) {
		...
		while (tick.getTime() <= this.end.getTime()) {
			...
			if(opts.tick){
				tick.setTime(Math.ceil(tick.getTime() + opts.tick*60*60*1000));
			}else{
	...
	render: function(opts) {
		...
		this.renderTimeWrap(oldScrollLeft, {tick: this.tick});
